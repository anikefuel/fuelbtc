...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-chwj03k1qold-api-Xa6JZq2055oa.gateway.appmedo.com/search` |
...
```json
{
  ...
  "pagination": {
    "current": 1,
    "next": "https://app-chwj03k1qold-api-Xa6JZq2055oa.gateway.appmedo.com/search?start=10"
  }
}
```
...
```bash
python3 google-scholar/scripts/call_google_scholar.py --q "deep learning" --as-ylo 2020 --as-yhi 2024
```
...
|----------|-------------|
| `--q` | Search keywords (supports `author:`, `source:` operators) |
| `--cites` | Article ID for citation retrieval |
| `--cluster` | Article cluster ID for related versions |
| `--as-ylo` | Start year (inclusive) |
| `--as-yhi` | End year (inclusive) |
| `--hl` | Interface language code (e.g. `en`, `zh-CN`) |
| `--lr` | Search language restriction |
| `--start` | Pagination offset (0, 10, 20, ...) |
| `--as-sdt` | Search scope: 0/7/4 |
| `--as-vis` | 0=all results, 1=reviews only |
| `--no-cache` | `true` to force real-time fetch |

At least one of `--q`, `--cites`, or `--cluster` is required.
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-chwj03k1qold-api-Xa6JZq2055oa.gateway.appmedo.com/search?${queryParams.toString()}`,
    {
      method: "GET",
      headers: {
