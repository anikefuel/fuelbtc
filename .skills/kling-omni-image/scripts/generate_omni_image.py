#!/usr/bin/env python3
"""
Submit a Kling omni-image task and poll until done.
...
SUBMIT_URL = "https://app-chwj03k1qold-api-2Y00Vzbe0MBY.gateway.appmedo.com/v1/images/omni-image"
QUERY_URL_BASE = "https://app-chwj03k1qold-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image"
POLL_INTERVAL_S = 7
SAFE_LIMIT_S = 550
...
def parse_args():
    ...
    p = argparse.ArgumentParser(description="Submit a Kling omni-image task and poll for the result.")
    ...
    p.add_argument("--resolution", choices=["1k", "2k"], help="output resolution")
    p.add_argument("-n", "--num", type=int, default=1, help="number of images to generate [1, 9]")
    p.add_argument("--mode", choices=["single", "series"], help="generation mode")
...
def submit_task(args, key):
    """Submit the omni-image task and return the task id."""
    payload = {
        "prompt": args.prompt,
...
def main():
    """Entry point: submit and poll the omni-image task, then print the result JSON."""
    args = parse_args()
    key = api_key()
    try:
