...
**Endpoint:** `GET https://app-chwj03k1qold-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image/{task_id}`
...
```bash
python3 <skill-path>/scripts/query_omni_image.py --task-id "<task_id>" [--output-dir /tmp/out]
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-chwj03k1qold-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image/${taskId}`,
    {
      method: "GET",
      headers: {
