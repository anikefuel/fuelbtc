...
// ── Web iframe fallback ───────────────────────────────────────────────────────
// Loads the widget via an in-memory HTML blob or srcDoc.
// This completely avoids hitting the Supabase Edge Function gateway from the browser iframe,
// preventing any "UNAUTHORIZED_NO_AUTH_HEADER" Supabase platform errors while ensuring
// camera permissions and SDK loading work smoothly.
function PremblyIframe({
  html, onClose, onMessage,
}: {
  html: string;
  onClose: () => void;
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[PremblyIframe] Failed to create blob URL:', e);
    }
  }, [html]);
...
export default function PremblyEmbedScreen() {
  ...
  // ── Web: use iframe with blob URL and srcDoc (no Supabase gateway auth error) ─
